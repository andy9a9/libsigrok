...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		GVariant *thresholds = std_gvar_min_max_step_thresholds(H4032L_THR_VOLTAGE_MIN,
				H4032L_THR_VOLTAGE_MAX, H4032L_THR_VOLTAGE_STEP);
		/* Take default threshold value from array */
		g_variant_get_child (thresholds, H4032L_THR_VOLTAGE_DEFAULT, "(dd)",
				     &devc->cur_threshold[0], &devc->cur_threshold[1]);
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_TRIGGER_MATCH:
		*data = std_gvar_array_i32(ARRAY_AND_SIZE(trigger_matches));
		break;
	case SR_CONF_VOLTAGE_THRESHOLD:
		*data = std_gvar_min_max_step_thresholds(H4032L_THR_VOLTAGE_MIN, H4032L_THR_VOLTAGE_MAX,
